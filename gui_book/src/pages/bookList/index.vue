...
<!--        </div>-->
<!--&lt;!&ndash;        <p class="bookPrice">{{item.price}}</p>&ndash;&gt;-->
<!--        <p class="bookRating">{{item.rating}} <Rate :value="item.rating"></Rate></p>-->
<!--      </div>-->
      <view class="cu-list2 menu-avatar2">
        <view class="cu-item2" :class="modalName=='move-box-'+ index?'move-cur2':''" v-for="(item,index) in booksArr" :key="index"
              @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + index" @click="toDetail(item.id)">
          ...
          <view class="content2" style="margin-left: 60rpx">
            ...
            <view class="text-orange text-df" v-if="item.rating"><Rate :value="item.rating?item.rating:item.user_rating"></Rate>{{item.rating}}</view>
...
<script>
    ...
    // import datas from '@/pages/index/datas/data.json'
    export default {
        components:{
          TopSwiper,Rate
        },
